...
@section('title') SoftwareClinic @endsection
    @section('content')
        ...
        <a href="{{ route('tipoprocrips.create') }}" class="btn btn-primary">Añadir Nuevo</a>
...
@endif
        ...
        <div class="card-body">
            <div class="col-sm-12">
                ...
                <tbody>
                @foreach($tipoprocrips as $estado)
                <tr>
                <td>{{ $estado->id }}</td>
                <td>{{ $estado->Nbre }}</td>
                ...
                <td>
                    <form action="{{ route('tipoprocrips.destroy', ['tipoprocrips' => $estado->id]) }}" method="POST" style="display:inline;">
                                 @csrf
                                 @method('DELETE')
