...
@section('title') SoftwareClinic @endsection
    ...
    @section('content')
        ...
        @slot('li_1') Interfaz de usuario @endslot
            ...
            @slot('li_3') Tipo Fin Consulta @endslot
            @slot('li_4') Añadir Nuevo @endslot
            @slot('title') Tipo Fin Consulta @endslot
        @endcomponent
                    ...
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Nuevo Tipo Fin Consulta</h4>
                                <p class="text-muted mb-0">Crea un nuevo Tipo Fin Consulta con la pantalla intutiva de SoftwareClinic.</p>
                                <div class="card-body">
                                <form method="POST" action="{{ route('tipofinproced.store') }}">
                                 @csrf
                                 <div class="mb-3">
                                    ...
                                    <div class="mb-3">
                                     <label for="Nbre" class="form-label">Tipo Fin Consulta</label>
                                 ...
                                 <div class="form-check">
                                     <input id="EsConsulta" type="checkbox" class="form-check-label @error('EsConsulta') is-invalid @enderror" name="EsConsulta" value="1" checked>
                                        @error('EsConsulta')
                                        <span class="invalid-feedback" role="alert">
                                         <strong>{{ $message }}</strong>
                                        </span>
                                        @enderror
                                        <label for="EsConsulta" class="form-check-label">Es Consulta</label>
                                     </div>
                                     <div class="mb-3">
                                 <div class="form-check">
                            ...
                            <button type="submit" class="btn btn-primary">{{ __('Guardar') }}</button>
                                <button onclick="location.href='{{ route('tipofinproced.index') }}'" class="btn btn-danger">Cancelar </button>
                            </div>
                        </div>
                    </form>
                </div>
