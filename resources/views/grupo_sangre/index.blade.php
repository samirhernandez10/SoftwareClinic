...
@section('title') SoftwareClinic @endsection
    @section('content')
        @component('components.breadcrumb')
            ...
            @slot('li_3') Grupo Sanguineo @endslot
            @slot('title')Grupo Sanguineo @endslot
        @endcomponent
        @section('css')
        ...
        <a href="{{ route('grupo_sangre.create') }}" class="btn btn-primary">Añadir Nuevo</a>
...
@endif
        ...
        <div class="card-body">
            <div class="col-sm-12">
                ...
                <tbody>
                @foreach($grupo_sangre as $estado)
                <tr>
                <td>{{ $estado->id }}</td>
                <td>{{ $estado->Nbre }}</td>
                <td>{{ $estado->activo ? 'No' : 'Si' }}</td>
                <td>{{ optional($estado->usuario)->name }}</td>

                <td>
                    <form action="{{ route('grupo_sangre.destroy', ['grupo_sangre' => $estado->id]) }}" method="POST" style="display:inline;">
                                 @csrf
                                 @method('DELETE')
