...
@section('title') SoftwareClinic @endsection
    @section('content')
        ...
        <a href="{{ route('salario_minimo.create') }}" class="btn btn-primary">Añadir Nuevo</a>
            ...
            <div class="col-sm-12">
                ...
                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                 <thead>
                    <tr>
                    <th>ID</th>
                    <th data-priority="3">Vr Salario Minimo</th>
                    <th data-priority="3">Año</th>
                    <th data-priority="3">Valor Diario</th>
                ...
                <tbody>
                @foreach($salario_minimo as $estado)
                <tr>
                <td>{{ $estado->id }}</td>
                <td>{{ $estado->valor }}</td>
                <td>{{ $estado->anio }}</td>
                <td>{{ $estado->valor_diario }}</td>
                <td>{{ $estado->estado ? 'Sí' : 'No' }}</td>
                <td>{{ optional($estado->usuario)->name }}</td>

                <td>
                    <form action="{{ route('salario_minimo.destroy', ['salario_minimo' => $estado->id]) }}" method="POST" style="display:inline;">
                                 @csrf
                                 @method('DELETE')
