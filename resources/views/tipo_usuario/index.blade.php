...
@section('content')
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-10">
                <div class="card">
                    <div class="card-header">Tipo Usuario</div>
                    <form action="{{ route('tipo_usuario.index') }}" method="GET">
                    <div class="mt-3">
                            <a href="{{ route('tipo_usuario.create') }}" class="btn btn-primary">Añadir Nuevo</a>
                        </div>
                            ...
                            <tbody>
                                @foreach ($tipo_usuario as $tipo_usuario)
                                    <tr>
                                        ...
                                        <td>{{ optional($tipo_usuario->usuario)->name }}</td>
                                        <td>
                                            <form action="{{ route('tipo_usuario.destroy', $tipo_usuario->id) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
