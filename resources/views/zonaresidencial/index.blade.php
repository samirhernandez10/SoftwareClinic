...
@section('title') SoftwareClinic @endsection
    @section('content')
        ...
        <a href="{{ route('zonaresidencial.create') }}" class="btn btn-primary">Añadir Nuevo</a>
...
@endif
        ...
        <div class="card-body">
            <div class="col-sm-12">
                ...
                <tbody>
                @foreach($zonaresidencial as $estado)
                <tr>
                <td>{{ $estado->id }}</td>
                <td>{{ $estado->nombre }}</td>
                ...
                <td>
                    <form action="{{ route('zonaresidencial.destroy', ['zonaresidencial' => $estado->id]) }}" method="POST" style="display:inline;">
                                 @csrf
                                 @method('DELETE')
