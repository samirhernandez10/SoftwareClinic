...
@section('content')
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-10">
                <div class="card">
                    <div class="card-header">Tipo Ambito</div>
                    <form action="{{ route('tipoambito.index') }}" method="GET">
                    <div class="mt-3">
                            <a href="{{ route('tipoambito.create') }}" class="btn btn-primary">Añadir Nuevo</a>
                        </div>
...
<table class="table datatable">
                            ...
                            <tbody>
                                @foreach ($tipoambito as $tipoambito)
                                    <tr>
                                        <td>{{ $tipoambito ->id }}</td>
                                        <td>{{ $tipoambito ->nombre }}</td>
                                        <td>{{ optional($tipoambito->usuario)->name }}</td>
                                        <td>
                                            <form action="{{ route('tipoambito.destroy', $tipoambito->id) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
