...
@section('title') SoftwareClinic @endsection
    ...
    @section('content')
        ...
        @slot('li_1') Interfaz de usuario @endslot
            ...
            @slot('li_3') Manual @endslot
            @slot('li_4') Añadir Nuevo @endslot
            @slot('title') Manual @endslot
        @endcomponent
        <style>
            body{
                    ...
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Nuevo Manual</h4>
                                <p class="text-muted mb-0">Crea un nuevo Manual con la pantalla intutiva de SoftwareClinic.</p>
                                <div class="card-body">
                                <form method="POST" action="{{ route('manual.store') }}">
                                 @csrf
                                 <div class="mb-3">
                                     <label for="nombre" class="form-label">Manual</label>
                            ...
                            <button type="submit" class="btn btn-primary">{{ __('Guardar') }}</button>
                                <button onclick="location.href='{{ route('manual.index') }}'" class="btn btn-danger">Cancelar </button>
                            </div>
                        </div>
                    </form>
                </div>
