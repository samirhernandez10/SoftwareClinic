...
@section('title') SoftwareClinic @endsection
    @section('content')
        ...
        <a href="{{ route('sexo.create') }}" class="btn btn-primary">Añadir Nuevo</a>
            ...
            <div class="col-sm-12">
                ...
                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                 <thead>
                    <tr>
                    <th>ID</th>
                    <th data-priority="1">Nombre sexo</th>
                ...
                @foreach ($sexo as $sex)
                    <tr>
                        <td>{{ $sex->id }}</td>
                        <td>{{ $sex->nombre }}</td>
                        <td>{{ $sex->activo ? 'Sí' : 'No' }}</td>
                         <td>{{ optional($sex->usuario)->name }}</td>
                         <td>
                         <form action="{{ route('sexo.destroy', ['sexo' => $sex->id]) }}" method="POST" style="display:inline;">
                                 @csrf
                                 @method('DELETE')
