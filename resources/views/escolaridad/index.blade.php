...
@section('title') SoftwareClinic @endsection
    @section('content')
        ...
        <a href="{{ route('escolaridad.create') }}" class="btn btn-primary">Añadir Nuevo</a>
        ...
        <div class="card-body">
            <div class="col-sm-12">
                ...
                <td>{{ $esc->id }}</td>
                <td>{{ $esc->nombre }}</td>
                <td>{{ $esc->activo ? 'Sí' : 'No' }}</td>
                <td>{{ optional($esc->usuario)->name }}</td>
                <td>
                    <form action="{{ route('escolaridad.destroy', ['escolaridad' => $esc->id]) }}" method="POST" style="display:inline;">
                                 @csrf
                                 @method('DELETE')
